...
const PurchaseReturnPrint = (selectedPurchaseReturnData: any[], currentCurrency: any) => {
  ...
  const returnsHTML = selectedPurchaseReturnData
    .map((purchaseReturn, index) => {
      const supplier =
      ...
      const productsHTML = purchaseReturnProducts
        .map(
          (prod, i) => `
          <tr class="${i % 2 === 0 ? "bg-white" : "bg-gray-50"}">
            ...
            <td class="border border-gray-300 px-3 py-2">${prod.productVariation?.color?.name || "-"}</td>
            <td class="border border-gray-300 px-3 py-2">${prod.productVariation?.size?.name || "-"}</td>
      ...
      const adjustmentHTML =
        purchaseReturn.totalAmount !== purchaseReturn.totalPaymentAmount
          ? `<p class="text-red-600 font-medium">
               <span class="font-semibold">Adjustment:</span> (${currentCurrency.name}) ${(
                 purchaseReturn.totalAmount - purchaseReturn.totalPaymentAmount
               )?.toFixed(2)}
             </p>`
          : "";
      ...
      return `
        <div class="p-6 no-break">
          <div class="text-center mb-4 border-b border-gray-300 pb-3">
            ...
            <h2 class="text-xl font-bold uppercase tracking-wide">Purchase Return</h2>
          </div>

          <div class="grid grid-cols-2 text-sm mb-4">
            <div>
              <p><span class="font-semibold">Return Challan No:</span> ${purchaseReturn.challanNo}</p>
              <p><span class="font-semibold">Supplier Name:</span> ${supplier}</p>
              <p><span class="font-semibold">Supplier Balance:</span> (${currentCurrency.name}) ${purchaseReturn.supplier?.balance?.toFixed(2) || "0.00"}</p>
            </div>
            <div class="text-right">
              <p><span class="font-semibold">Return Date:</span> ${date}</p>
              <p><span class="font-semibold">Payment Account:</span> ${purchaseReturn.account?.accountType}</p>
              <p><span class="font-semibold">Account Balance:</span> (${currentCurrency.name}) ${purchaseReturn.account?.balance?.toFixed(2) || "0.00"}</p>
            </div>
          </div>
          ...
          <div class="text-right text-sm space-y-1 mb-6">
            <p><span class="font-semibold">Total Return Amount:</span> (${currentCurrency.name}) ${purchaseReturn.totalAmount?.toFixed(2)}</p>
            <p class="text-green-600 font-medium">
              <span class="font-semibold">Refund Amount:</span> (${currentCurrency.name}) ${purchaseReturn.totalPaymentAmount?.toFixed(2)}
            </p>
            ${adjustmentHTML}
            <p class="border-t pt-1 mt-1 font-semibold">
              <span class="font-semibold">Supplier New Balance:</span> (${currentCurrency.name}) ${(purchaseReturn.supplier?.balance - purchaseReturn.totalPaymentAmount)?.toFixed(2)}
            </p>
          </div>
        </div>
        ${index < selectedPurchaseReturnData.length - 1 ? "<div class='page-break'></div>" : ""}
      `;
    })
    .join("");
