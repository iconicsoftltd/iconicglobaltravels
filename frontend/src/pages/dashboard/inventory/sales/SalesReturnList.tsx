...
export default function SalesReturnList() {
  ...
  const [selectedReturn, setSelectedReturn] = useState<SalesReturnType | null>(
    null,
  );
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAllSalesReturnQuery({ page: page + 1, size: rowsPerPage, search });
  ...
  const salesReturnColumns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        `${currentCurrency.name} ${row.original.totalPaymentAmount?.toFixed(2) || "0.00"}`,
    },
    {
      accessorKey: "date",
      header: "Date",
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="bg-gray-100"
              size="icon"
              onClick={() => navigate(`/create-sales-return?id=${item.id}`)}
            >
              <FaRegEdit />
            </Button>
    ...
    <>
      <section className="min-h-screen space-y-4 p-4">
        ...
        <ReusableTableHeader
          ...
          createButtonLink="/create-sales-return"
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto scrollbar-hide">
          <div className="flex flex-col h-full">
            <Heading>Sales Return Details</Heading>
